...
module.apps = spotlight.new():queryString([[ kMDItemContentType = "com.apple.application-bundle" ]]):callbackMessages(
                'didUpdate', 'didFinish'):setCallback(function(obj, msg, info)
  if module.chooser then module.chooser:refreshChoicesCallback() end
end):start()

module.chooser = chooser.new(function(choice) if choice then application.launchOrFocus(choice.path) end end)
module.chooser:fgColor{list='x11', name='lightskyblue'}:subTextColor{list='x11', name='darkseagreen'}:width(60)
  :searchSubText(true):choices(function()
    local results = {}
      ...
      if module.apps[i] then
        ...
        if path then
          table.insert(results, {
            ...
            image=bid and image.imageFromAppBundle(bid)
              or image.imageFromName(image.systemImageNames.StopProgressFreestandingTemplate),
            index=i,
            path=path,
            lastOpened=usedDate
          })
    ...
    if not next(rowDetails) then
      print('not in apps');
      return
    end -- empty table means right click not on an actual item
    local menuItems = {}
    for i, v in ipairs(module.apps[rowDetails.index]:attributes()) do
      ...
      table.insert(menuItems, {
        ...
        menu={
          {title=styledText(value, {font={name='Menlo', size=10}}), fn=function() pasteboard.setContents(value) end}
        }
      })
    end
    table.insert(menuItems, {
