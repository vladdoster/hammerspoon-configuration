...
local batteryPowerSource = function()
  return battery.powerSource() or 'no battery'
end
...
module.batteryNotifications = {
  {
    ...
    fn = function()
      ...
      if not suppressAudio then
        ...
        if volume then
          audio:setVolume(25)
        end -- apparently some devices don't have a volume or mute...
        if muted then
          audio:setMuted(false)
        end
        local sp = speech.new('Zarvox')
    ...
    if not justOn then
      for i, v in ipairs(module.batteryNotifications) do
        if v.onBattery == test.onBattery then
          ...
          if notificationStatus[i] then
            notificationStatus[i] = nil
          end
        end
      end
    else
...
local displayBatteryData = function(modifier)
  ...
  table.insert(menuTable, {
    title = utf8.codepointToUTF8(0x1F340) .. '  Battery Health: ' .. (maxCapacity and designCapacity and string.format(
      '%.2f%%',
...
module = setmetatable(module, {
  __gc = function(self)
    ...
    if module.menuTitleChanger then
      module.menuTitleChanger:stop()
    end
  end,
})
