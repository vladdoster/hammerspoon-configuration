...
local prompt = require('ext.prompter').prompt
local settingsTag = 'ext.clipboard'
...
local prepareInitialHistory = function(rawHistory)
  ...
  for _, v in ipairs(rawHistory) do
    local key = hashFN(v)
...
local saveHistoryAndSettings = function(theHistory)
  ...
  if theHistory then setSetting('clipHistory', theHistory.history) end
end
...
local shouldRecordContents = function()
  local goAhead = true
  for _, v in ipairs(pb.pasteboardTypes()) do
  ...
  if goAhead then
    for _, v in ipairs(pb.contentTypes()) do
      if ignoredIdentifiers[v] then
        goAhead = false
        break
      end
  ...
  if shouldRecordContents() and theClipping then
    ...
    if theHistory.hashed[hashValue] then
      ...
      while overage > 0 do
        ...
        for k, _ in pairs(theHistory.hashed) do theHistory.hashed[k] = theHistory.hashed[k] - 1 end
  ...
  if not eventtap.checkMouseButtons().right then
    ...
    if #primaryClipHistory.history == 0 then
      ...
      table.insert(results, {
        title=stext.new('Hold down ' .. utf8.registeredKeys.alt .. ' to ' .. utf8.registeredKeys.leftDoubleQuote
                          .. 'type' .. utf8.registeredKeys.rightDoubleQuote .. ' selection immediately', {
      ...
      table.insert(results, {
        title=stext.new('Hold down ' .. utf8.registeredKeys.ctrl .. ' to remove selection from history', {
          font={name=stext.defaultFonts.menu.name .. 'Italic', size=stext.defaultFonts.menu.size},
          color={list='x11', name='royalblue'}
        }),
        disabled=true
      ...
      for i = #primaryClipHistory.history, 1, -1 do
        local itemTitle = (#primaryClipHistory.history[i] > labelLength)
                            and primaryClipHistory.history[i]:sub(1, labelLength) .. ' '
                            .. utf8.codepointToUTF8('U+2026') or primaryClipHistory.history[i]
        table.insert(results, {title=itemTitle, fn=function() itemSelected(i, primaryClipHistory) end})
      end
    end
  else
    -- do special menu
    table.insert(results, {
      title=stext.new('newClipper Options', {
        font={name=stext.defaultFonts.menu.name .. 'Italic', size=stext.defaultFonts.menu.size},
        color={list='x11', name='royalblue'}
      }),
      disabled=true
    ...
    table.insert(results, {
      title='Maximum item size: ' .. tostring(maxSize) .. ' byte(s)',
      fn=function()
        prompt('Maximum clipboard item size to save in history in bytes:', maxSize, function(input)
          if input then
    ...
    table.insert(results, {
      title='Polling frequency: ' .. tostring(frequency) .. ' second(s)',
      fn=function()
        prompt('Clipboard polling frquency in seconds:', frequency, function(input)
          if input then
            if tonumber(input) then
    ...
    table.insert(results, {
      title='Maximum items saved: ' .. tostring(historySize),
      fn=function()
        prompt('Maximum number of items to save in history:', historySize, function(input)
          if input then
            if tonumber(input) then
    ...
    table.insert(results, {
      title='Maximum label length: ' .. tostring(labelLength),
      fn=function()
        prompt('Maximum number of characters to display for each item:', labelLength, function(input)
          if input then
            if tonumber(input) then
    ...
    table.insert(results, {
      title='Type selection instead of updating clipboard',
      checked=typeOnSelect,
      fn=function()
    ...
    table.insert(results, {
      title='Also update clipboard when typing',
      checked=typeAlsoUpdates,
      fn=function()
    ...
    table.insert(results, {title='-'})
    table.insert(results,
                 {title='Clear history', fn=function() primaryClipHistory = clearHistory(primaryClipHistory) end})
    table.insert(results, {title='Save history now', fn=function() saveHistoryAndSettings(primaryClipHistory) end})
  end

  table.insert(results, {title='-'})
  table.insert(results, {
    title=stext.new('newClipper for Hammerspoon', {
      font={name=stext.defaultFonts.menu.name .. 'Italic', size=stext.defaultFonts.menu.size},
      color={list='x11', name='royalblue'},
