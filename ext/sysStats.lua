...
obj.updateDisplay = function(cpuUsage)
    ...
    local cpuActive = math.floor(cpuUsage.overall.active + 0.5)
    if cpuActive < 100 then
        cpuGuage = cpuGuage:setStyle({ color = obj.emptyBar }, math.ceil(obj.barWidth * cpuActive / 100), #cpuGuage)
    end

    cpuGuage = stext.new("CPU ", { font = obj.baseFont, color = { list = "ansiTerminalColors", name = "fgBlack" } })
        .. cpuGuage
    ...
    ramGuage = stext.new("RAM ", { font = obj.baseFont, color = { list = "ansiTerminalColors", name = "fgBlack" } })
        .. ramGuage

    local active = math.floor(100 * percentInUse)
    ramGuage = ramGuage
        .. stext.new(string.format(" %3d%% used, %.2fGB free", active, totalFree), {
            font = obj.baseFont,
            color = {
                list = "ansiTerminalColors",
    ...
    if max_charge and cur_charge then
        ...
        batteryGuage = batteryGuage
            .. stext.new(string.format(" %3d%% charged, %d mAh", avail, cur_charge), {
                font = obj.baseFont,
                color = {
                    list = "ansiTerminalColors",
    ...
    -- styledtext concatenation drops the style of text after a line break; this works around it
    ...
    -- checkInterval may change between calls, and :hide may have run since this was triggered
    if obj._timer then
        obj._timer:setNextTrigger(obj.checkInterval)
    end
end
...
obj.cornerRadius = 5
obj.backgroundColor = { alpha = 0.7, white = 0.5 }
